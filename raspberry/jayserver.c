...
int   main( void)
{
		...
		if(menu==1)
		{
				...
				while( 1)
				{
						if( -1 == listen(server_socket, 5)){    /* What do you mean of the parameter number 5? */
                                                                /* the second parameter is backlog, is the maximum length that the queue of
                        ...
                        /* Accept */
                        /* is this just waiting for a client trying to connect to server?*/
                        /* Actually I don't know how this function works. */
								...
								if(!strcmp(sArr[0],readbuff))
								{
										...
										while(fgets(readbuff,18,fp2))
										{
												if(!strcmp(sArr[1],readbuff))
												{
                                                        ...
                                                        /* Why does it put the phone number received to this PhoneList.txt?? */
